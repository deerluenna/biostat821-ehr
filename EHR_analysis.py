...
conn = sqlite3.connect("EHR.db")  # Create filename
print(conn)
...
cur.execute("CREATE UNIQUE INDEX idx_patient_id ON patient (PatientID)")
...
with open("Patient.txt", "r", encoding="utf-8-sig") as patient_file:
    ...
    for line in text:
        ...
        # print(a, b, c, d, e, f, g)
        cur.execute(
            "INSERT INTO patient VALUES('{a}','{b}','{c}','{d}','{e}','{f}','{g}')"
        )

with open("Lab.txt", "r", encoding="utf-8-sig") as lab_file:
    for line in text:
        col_in_line = line.strip().split("\t")
        a, b, c, d, e, f, g = col_in_line
        # print(a, b, c, d, e, f, g)
...
cur.execute("CREATE INDEX IF NOT EXISTS ID ON patient(PatientID)")


class Patient:
    """Create a patient class."""

    def __init__(self, cursor, pat_id):
        """Initialize."""
        self.cursor = cursor
        self.pat_id = pat_id
        ...
        # Fetch gender info
        cur.execute(
            f"SELECT patient.PatientGender FROM patient WHERE PatientID = '{self.pat_id}'"
        )
        self.gender = cur.fetchall()[0][0]

        # Fetch DOB info
        cur.execute(
            f"SELECT patient.PatientDateOfBirth FROM patient WHERE PatientID = '{self.pat_id}'"
        )
        self.DOB = cur.fetchall()[0][0]

        # Fetch Race info
        cur.execute(
            f"SELECT patient.PatientRace FROM patient WHERE PatientID = '{self.pat_id}'"
        )
