...
def test_parse_data():
    ...
    assert parse_data(filename_patient)[
        1][0] == 'F0B53A2C-98CA-415D-B928-E3FD0E52B22A'
    assert parse_data(filename_patient)[
        2][0] == '1A8791E3-A61C-455A-8DEE-763EB90C9B2A'
...
def test_sick_patients():
    ...
    assert sick_patients('METABOLIC: CALCIUM', "<",
                         '8.5', parsed_lab_data) == ['1A8791E3-A61C-455A-8DEE-763EB90C9B2A']
    assert sick_patients('METABOLIC: GLUCOSE', "<",
                         '126.0', parsed_lab_data) == ['1A8791E3-A61C-455A-8DEE-763EB90C9B2A']
    assert sick_patients('METABOLIC: GLUCOSE', ">",
                         '126.0', parsed_lab_data) == 'No one is larger'
...
def test_age_first_adm():
    ...
    Test_Patient_1 = 'Test_Patient_1.txt'
    parsed_patient_data = parse_data(Test_Patient_1)
    assert age_first_adm(parsed_patient_data, parsed_lab_data,
                         '1A8791E3-A61C-455A-8DEE-763EB90C9B2A') == 12
    assert age_first_adm(parsed_patient_data, parsed_lab_data,
                         '1A8791E3-A61C-455A-8DEE-763EB90C9B2B') == 22
